...
class GradAlign:
    ...
    def run_algorithm(self): #anchor is not considered yet
       ...
       # model = myGCN(len(self.att_s.T), hidden_channels=self.hid_channel, num_layers = self.layer)
        ...
        S_prime, result,pre_dict = self.FinalEvaluation(S, embedding1, embedding2, seed_list1, seed_list2, self.idx1_dict, self.idx2_dict, adj2)
        return pre_dict
        ...
        def create_dict_from_lists(keys: list, values: list) -> dict:
            ...
            异常:
                如果两个列表长度不同，抛出ValueError
            """
            if len(keys) != len(values):
                raise ValueError("两个列表长度必须相同")

            return dict(zip(keys, values))
        pre_align_dict=create_dict_from_lists(seed_list1,seed_list2)
        return pre_align_dict
    ...
    #
    #     if self.eval_mode == True:
    #         adj2 = calculate_Tversky_coefficient_final(self.G1, self.G2, seed_list1, seed_list2, index, columns,
    ...
    #
    #     # 计算MRR指标
    #     mrr = self.compute_mrr(S, gt_dict, idx1_dict, idx2_dict)
    ...
    #     print('Success@10 : {:.4f}'.format(top10_eval))
    #
    #     result = '@1:' + str(round(top1_eval, 4)) + ',  @5:' + str(round(top5_eval, 4)) + ',  @10:' + str(
    #         round(top10_eval, 4)) + ',  Acc:' + str(round(acc, 4)) + ',  MRR:' + str(round(mrr, 4))
    #
    #     return S, result
    def FinalEvaluation(self, S, embedding1, embedding2, seed_list1, seed_list2, idx1_dict, idx2_dict, adj2):
        count = 0
        ...
        # 创建反向映射：索引到节点名称
        idx_to_node1 = {v: k for k, v in idx1_dict.items()}
        idx_to_node2 = {v: k for k, v in idx2_dict.items()}
        ...
        # 创建Success@1节点对字典
        success1_dict = {}
        for i in range(top_1.shape[0]):
            # 找到预测的目标节点索引
            j = np.argmax(top_1[i])
            if top_1[i, j] == 1:  # 确保是Top1预测
                # 获取源节点名称
                source_node = idx_to_node1.get(i, None)
                # 获取目标节点名称
                target_node = idx_to_node2.get(j, None)

                if source_node is not None and target_node is not None:
                    success1_dict[source_node] = target_node

        # 计算Success@1指标
        top1_eval = compute_precision_k(top_1, gt_dict, idx1_dict, idx2_dict)

        # 计算其他指标（Top5, Top10, MRR等）
        top_5 = top_k(S, 5)
        top_10 = top_k(S, 10)
        ...
        # 返回Success@1节点对字典和其他结果
        return S, result, success1_dict

    def compute_mrr(self, S, gt_dict, idx1_dict, idx2_dict):
        """
        计算平均倒数排名 (Mean Reciprocal Rank, MRR)

        参数:
            S: 相似度矩阵
            gt_dict: 真实对齐字典 {G1节点: G2节点}
            idx1_dict: G1节点到索引的映射
            idx2_dict: G2节点到索引的映射
        ...
        # 遍历所有G1节点
        for g1_node, g2_gt_node in gt_dict.items():
            if g1_node not in idx1_dict:
                continue

            g1_idx = idx1_dict[g1_node]
            g2_gt_idx = idx2_dict.get(g2_gt_node, -1)

            if g2_gt_idx == -1:
                continue

            # 获取该G1节点对所有G2节点的相似度
            similarities = S[g1_idx]

            # 按相似度降序排序索引
            sorted_indices = np.argsort(similarities)[::-1]

            # 找到真实对齐节点的排名
            rank = np.where(sorted_indices == g2_gt_idx)[0]

            if len(rank) > 0:
                rank = rank[0] + 1  # 排名从1开始
