...
# from sklearn.metrics import roc_auc_score, cosine_similarity
...
#
#         if args.graphname == 'am-td':
#             # 特殊处理am-td数据集
#             source_dataset = Dataset(os.path.join(args.data_folder, 'allmv_tmdb', 'allmv'))
#             target_dataset = Dataset(os.path.join(args.data_folder, 'allmv_tmdb', 'tmdb'))
#             gt_dr = os.path.join(args.data_folder, 'allmv_tmdb', 'dictionaries', 'groundtruth')
...
#
#             # 预处理图和对齐字典
#             G1, G2, gt_dict, gt_dict_reversed = preprocessing(G1, G2, gt_dict)
#
#             # 创建索引映射
#             idx1_dict, idx2_dict = create_idx_dict_pair(G1, G2, gt_dict)
#
#             return G1, G2, attr1, attr2, gt_dict, gt_dict_reversed, idx1_dict, idx2_dict
#
#         elif args.graphname == 'fl-my':
#             # 特殊处理fl-my数据集
#             source_dataset = Dataset(os.path.join(args.data_folder, 'fl-my', 'flickr'))
#             target_dataset = Dataset(os.path.join(args.data_folder, 'fl-my', 'myspace'))
#             gt_dr = os.path.join(args.data_folder, 'fl-my', 'dictionaries', 'groundtruth')
#             gt_dict = graph_utils.load_gt(gt_dr, source_dataset.id2idx, target_dataset.id2idx, 'dict')
#
#             # 处理属性
#             attr1, attr2, attribute_sim = AttributeProcessing(args, G1, G2, gt_dict)
...
#
#             # 预处理图和对齐字典
#             G1, G2, alignment_dict, alignment_dict_reversed = preprocessing(G1, G2, alignment_dict)
#
#             # 创建索引映射
...
#     '''
#     预处理函数，使两个图的节点ID都从0开始连续
#
#     参数:
#         G1 : 源图
#         G2 : 目标图
#         alignment_dict : 真实对齐字典
#
#     返回:
#         G1, G2: 重映射后的图
#         new_alignment_dict: 更新后的对齐字典
#         new_alignment_dict_reversed: 反向对齐字典
#     '''
#     # 创建G1的映射：原始节点ID -> 新ID (0到n1-1)
#     G1_nodes = sorted(list(G1.nodes()))
#     G1_mapping = {old: new for new, old in enumerate(G1_nodes)}
#     G1 = nx.relabel_nodes(G1, G1_mapping)
#
#     # 创建G2的映射：原始节点ID -> 新ID (0到n2-1)
#     G2_nodes = sorted(list(G2.nodes()))
#     G2_mapping = {old: new for new, old in enumerate(G2_nodes)}
#     G2 = nx.relabel_nodes(G2, G2_mapping)
#
#     # 更新对齐字典
#     new_alignment_dict = {}
#     for g1_old, g2_old in alignment_dict.items():
#         # 确保节点在映射中
#         if g1_old in G1_mapping and g2_old in G2_mapping:
#             new_g1 = G1_mapping[g1_old]
#             new_g2 = G2_mapping[g2_old]
#             new_alignment_dict[new_g1] = new_g2
#
#     # 创建反向字典
#     new_alignment_dict_reversed = {v: k for k, v in new_alignment_dict.items()}
#
#     return G1, G2, new_alignment_dict, new_alignment_dict_reversed
#
#
...
#
#     # 导出文件
#     nx.write_edgelist(G3, f"{graphname}1_ran{rand_portion}.edges", delimiter=',', data=False)
#     nx.write_edgelist(G4, f"{graphname}2_ran{rand_portion}.edges", delimiter=',', data=False)
#     print('导出数据完成')
#
#     return G3, G4, Ggt
#
...
#     处理属性数据
#     """
#     attribute_sim, attr1, attr2, attr1_pd, attr2_pd = read_attribute(args.attribute_folder, args.graphname, G1, G2,
#                                                                      alignment_dict)
#
#     # 保存属性数据
...
#
#     for _ in range(num_mask_common):
#         e = random.sample(list(edgelist1), 1)
#         start_vertex = e[0][0]
#         end_vertex = e[0][1]
#
#         try:
...
#
#     for _ in range(num_mask_rand):
#         e = random.sample(list(edgelist2), 1)
#         start_vertex = e[0][0]
#         end_vertex = e[0][1]
#
...
#         self._load_G()
#         self._load_features()
#         graph_utils.construct_adjacency(self.G, self.id2idx, sparse=False,
#                                         file_path=os.path.join(self.data_dir, "edges.edgelist"))
#         print("数据集信息:")
#         print("- 节点数: ", len(self.G.nodes()))
#         print("- 边数: ", len(self.G.edges()))
#
#     def _load_G(self):
#         with open(os.path.join(self.data_dir, "G.json")) as f:
#             G_data = json.load(f)
#         G_data['links'] = [{'source': self.idx2id[G_data['links'][i]['source']],
#                             'target': self.idx2id[G_data['links'][i]['target']]}
...
#     读取对齐文件
#     """
#     alignment = pd.read_csv(os.path.join(alignment_folder, filename + '.csv'), header=None)
#     alignment_dict = {}
...
#     G1 = nx.Graph()
#     G2 = nx.Graph()
#     G1_edges = pd.read_csv(os.path.join(data_folder, filename + '1.edges'), names=['0', '1'])
#     G1.add_edges_from(np.array(G1_edges))
#     G2_edges = pd.read_csv(os.path.join(data_folder, filename + '2.edges'), names=['0', '1'])
...
#     G2_nodes = sorted(list(G2.nodes()))
#
#     attribute1_pd = pd.read_csv(os.path.join(attribute_folder, filename + 'attr1.csv'), header=None, index_col=0)
#     attribute2_pd = pd.read_csv(os.path.join(attribute_folder, filename + 'attr2.csv'), header=None, index_col=0)
#
...
#     G2_nodes = sorted(list(G2.nodes()))
#
#     # 创建G1的映射：节点ID -> 索引 (0到n1-1)
#     idx1_dict = {node: idx for idx, node in enumerate(G1_nodes)}
#
#     # 创建G2的映射：节点ID -> 索引 (0到n2-1)
#     idx2_dict = {node: idx for idx, node in enumerate(G2_nodes)}
#
...
#     """
#     # 测量最大度
#     max_deg = max(max(dict(Gs.degree).values()), max(dict(Gt.degree).values()))
#     print(f"最大度: {max_deg}")
#
#     # 计算属性数量
...
#     基于k-hop增强属性
#     """
#     Gs_nodes = sorted(list(Gs.nodes()))
#     Gt_nodes = sorted(list(Gt.nodes()))
#
#     # 计算k-hop邻居
#     khopdict_source = {node: len(nx.single_source_shortest_path_length(Gs, node, cutoff=k))
#                        for node in Gs_nodes}
#     khopdict_target = {node: len(nx.single_source_shortest_path_length(Gt, node, cutoff=k))
#                        for node in Gt_nodes}
#
#     # 测量最大度
#     max_deg = max(max(khopdict_source.values()), max(khopdict_target.values()))
#     print(f"最大k-hop度: {max_deg}")
#
#     # 计算属性数量
#     num_attr = math.ceil(max_deg / interval)
#
...
#
#     # 为每个节点分配属性
#     for idx_s, node_s in enumerate(Gs_nodes):
#         deg_node = khopdict_source[node_s]
#         init_np_s[idx_s, int(deg_node / interval) - 1] = 1
#
#     for idx_t, node_t in enumerate(Gt_nodes):
...
#
#     # 缩放值
#     katzdict_s = {k: v * mul for k, v in katzdict_s.items()}
#     katzdict_t = {k: v * mul for k, v in katzdict_t.items()}
#
#     # 测量最大长度
#     max_len = max(max(katzdict_s.values()), max(katzdict_t.values()))
#     print(f"属性长度: {max_len}")
#
#     # 计算属性数量
...
#         idx1, idx2 = random.sample(range(0, len(att)), 2)
...
#
#     # 计算度
#     degdict_G1 = {node: G1.degree(node) for node in G1_nodes}
#     degdict_G2 = {node: G2.degree(node) for node in G2_nodes}
#
#     # 比较对齐字典
#     score = 0
...
#         try:
#             if np.array_equal(attr1[idx1_dict[node]], attr2[idx2_dict[alignment_dict[node]]]):
#                 score += 1
#         except:
#             continue
#
#     att_consist = score / len(alignment_dict)
#     print(f"相同属性比例: {att_consist:.2f}")
#
#     return att_consist
